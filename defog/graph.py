...
def arr_to_image(img_work, fname):
    ...
    im.save(fname) 
    return 0
def calculate_energy(img_work,prior_maps : np.ndarray, lambda_: float, threshold: float, v_max: float, sigma : np.ndarray):
...
def alpha_beta_swap_new(alpha, beta, img_work, prior_maps: np.ndarray, lambda_: float, threshold: float, v_max: float,sigma : np.ndarray):
    ...
    for i in range(0,len(map)):
        ...
        t_weight_alpha = sum([V_p_q(alpha,v, lambda_, threshold, v_max) for v in fil_neigh]) + D_p(alpha, prior_maps, x, y, sigma)
        t_weight_beta = sum([V_p_q(beta,v, lambda_, threshold, v_max) for v in fil_neigh]) + D_p(beta, prior_maps, x, y, sigma)
...
def expansion_move(img_orig, img_work, prior_maps:np.ndarray, cycles,lambda_: float, threshold: float, v_max: float,sigma :np.ndarray):
    ...
    for u in range(0,cycles):
        ...
        for i in range(0, len(labels)-1):
            for j in range(i+1, len(labels)):
                ...
                img_work  = alpha_beta_swap_new(labels[i],labels[j], img_work,prior_maps,lambda_,threshold,v_max,sigma)     
