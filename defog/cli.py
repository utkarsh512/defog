...
def main():
    ...
    cv2.imwrite("depth_map_init.jpg", make_image(depth_map.copy()))
    ...
    depth_map = optimizer.optimize(depth_map=depth_map,
                                   ...
                                   v_max=args.v_max)
    cv2.imwrite("depth_map_final.jpg", make_image(depth_map.copy()))
    print(f"[DONE] Depth map optimized.")
    depth_map_discrete = make_image(depth_map.copy())
    depth_map = np.exp(-depth_map)
    print(f"[*] Estimating airlight...")
    edge_image = airlight.get_edge_image(ip_image)
    fog_region = airlight.fog_opaque_region(edge_image, depth_map_discrete)
    ...
    for i in range(h):
        for j in range(w):
            for k in range(c):
                atm_image[i, j, k] = (1 - depth_map[i, j]) * atm_lum[k]
...
def run_as_command():
    ...
    if float(version) < 3.7:
        print("[-] defog requires Python version 3.7+.")
        sys.exit(0)
    main()
